...
war = random.choice([P,B,Z])
banner = f"""{war}
  ____________  ___                  __             __
 /_  __/_  __/ / _ \___ _    _____  / /__  ___ ____/ /
  / /   / /   / // / _ \ |/|/ / _ \/ / _ \/ _ `/ _  /
 /_/   /_/   /____/\___/__,__/_//_/_/\___/\_,_/\_,_/
   {M}• {P}Author {M}: {P}Avin Yetee!!
   {M}• {P}Github {M}: {K}/NakanoYetee
   {M}• {P}Team   {M}: {H}Team AvinYetee
 {M}• {P}Masukan Link Video Tiktok {M}({H}https://vt.tiktok.com/bukansekali{M})"""

path = "/storage/emulated/0/Tiktok-Download-AvinYete"
try:
        os.mkdir(path)
except:
        pass
...
class Main:
	...
	def get_dat(self, url):
		...
		try:
			...
			if 'true' in data:
				...
				gas = input(f'\n {M}• {P}Lanjut Untuk Download ?.. {M}[{H}Y/T{M}]:{H} ')
				if gas in ['Y','y']:
					nama_vid = input(f'\n {M}• {P}Masukan Nama Video Kamu {M}[{P}OUTPUT{M}]{M}:{H} ')
					self.dow_tt(tk_vid, id_vid, nama_vid)
				else:
					exit(f' {M}• {P}Thanks udah pake jangan lupa subscribe Yt Avin Yetee!!')
	...
	def mulai(self):
		...
		if url in ['']:
			exit(f' {M}• {P}Masukan link yg benar cox -_-')
		else:
			self.get_dat(url)
Main().mulai()